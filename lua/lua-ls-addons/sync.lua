...
---@return string|nil release_version The latest release version tag, or nil if failed.
		...
		if release.assets then
			for _, asset in ipairs(release.assets) do
				if asset.name:match(pattern) then
					-- Strictly internal tag_name for fetching
...
---@return string|nil actual_version The resolved version string (release tag).
---@return string|nil pretty_name The display name of the release.
local function download_release(repo, target_dir, version, pattern)
	...
	local pretty = (release.name and release.name ~= "") and release.name or release.tag_name
	return true, release.tag_name, pretty
end
...
function M.ensure_installed(parsed, locked_version, force_update, custom_interval)
	...
	local success, actual_ver, pretty_name = false, nil, nil
	if parsed.is_release then
		success, actual_ver, pretty_name =
			download_release(parsed.repo, target_dir, target_version, parsed.release_pattern)
	...
	if success then
		-- Use pretty release name for logs, or fallback to short commit hash/tag
		local display_ver = pretty_name or (target_version and target_version:sub(1, 7)) or "unknown"
		utils.log_info(string.format("Installed %s (%s)", parsed.original_name, display_ver), "")
		return target_dir, target_version
	else
